...
import { Plus, Loader2, Search as SearchIcon, ChevronRight, Receipt } from "lucide-react";
...
const InvoiceListPage = () => {
  ...
  // Invoices are paginated (12/page) from the backend, but the search box
  // needs to match across every page, not just the ones already loaded —
  // so while searching we fetch the full invoice list separately and filter
  // over that instead of over the current page's `invoices`.
  ...
  // Debounced fetch of the full invoice list whenever there's an active
  // search term, so results aren't limited to the currently loaded page.
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await InvoiceService.getAllInvoices(1, totalItems || 100000);
        if (!cancelled) setSearchResults(res.invoices || []);
      } catch (error) {
        console.error("Failed to fetch invoices for search:", error);
  ...
  const groupedUsers = useMemo(() => {
    ...
    // While searching, filter across every invoice (searchResults), not just
    // the ones on the currently loaded page (invoices).
    const sourceInvoices = trimmed ? searchResults ?? [] : invoices;

    const filtered = trimmed
      ? sourceInvoices.filter(
          (inv) =>
            inv.billTo?.companyName?.toLowerCase().includes(trimmed) ||
            inv.billTo?.contactPerson?.toLowerCase().includes(trimmed)
        )
      : sourceInvoices;
  ...
  }, [invoices, searchResults, searchTerm]);
...
const handleDownloadCustomerReport = async (user) => {
  ...
  if (loading) {
    return (
      <div className="flex min-h-[400px] flex-col items-center justify-center bg-gradient-to-b from-orange-50/50 to-white">
        <Loader2 className="mb-2 animate-spin text-orange-500" size={40} />
        <p className="font-medium text-gray-500">Loading Database...</p>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-800 sm:text-2xl">Customer Invoices</h1>
              <p className="text-sm text-gray-500">
                Manage billing records
                {totalItems > 0 && (
                  <span className="ml-1.5 font-semibold text-orange-500">· {totalItems} total</span>
                )}
              </p>
            </div>
          </div>
        ...
        {/* ── Search ── */}
        <InvoiceSearch searchTerm={searchTerm} setSearchTerm={setSearchTerm} isSearching={isSearching} />
        ...
        <div className="space-y-4">
          {groupedUsers.length > 0 ? (
            ...
            <div className="rounded-3xl border border-dashed border-orange-200 bg-orange-50/30 p-12 text-center sm:p-20">
              <SearchIcon className="mx-auto mb-4 text-orange-200" size={48} />
        ...
        {/* ── Pagination ── */}
        {/* Hidden while searching — results are pulled from the full invoice
            list at that point, so page-by-page navigation doesn't apply. */}
          ...
          <div className="mt-8 flex flex-col items-center gap-4">
            <div className="flex flex-wrap items-center justify-center gap-3">
              <button
                ...
                className={`flex items-center gap-1.5 rounded-xl px-3.5 py-2 font-medium shadow-sm transition-all sm:px-4
                  ${currentPage === 1
                    ? "cursor-not-allowed bg-gray-100 text-gray-400"
                    : "border border-orange-200 bg-white text-gray-700 hover:bg-orange-50 hover:text-orange-600"
                  }`}
              >
